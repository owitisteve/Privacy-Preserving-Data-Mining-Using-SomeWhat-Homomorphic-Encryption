...
require 'db.php'; // Database connectioning notifications
$registration_number = $_SESSION['registration_number']; // New addition
$email = null;

// Try to get email from approvals table first
$check_approval = $conn->prepare("SELECT email FROM approvals WHERE registration_number = ?");
$check_approval->bind_param("s", $registration_number);
$check_approval->execute();
$approval_result = $check_approval->get_result();

if ($approval_result->num_rows > 0) {
    $email = $approval_result->fetch_assoc()['email'];
} else {
    // Fallback to counselees table
    $check_counselee = $conn->prepare("SELECT email FROM counselees WHERE registration_number = ?");
    $check_counselee->bind_param("s", $registration_number);
    $check_counselee->execute();
    $counselee_result = $check_counselee->get_result();

    if ($counselee_result->num_rows > 0) {
        $email = $counselee_result->fetch_assoc()['email'];
    } else {
